...
const getStats = async (req, res, next) => {
  try {
    ...
    const totalWeapons = await Notification.countDocuments({ type: 'weapon', ...userScope });
    const alertsSent = await Notification.countDocuments({ ...userScope });
...
const getActivity = async (req, res, next) => {
  try {
    ...
    const recentNotifications = await Notification.find(userScope)
      .sort({ createdAt: -1 })
      .limit(10)
      .lean();
